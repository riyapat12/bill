...
class RptCollection_Controller extends CI_Controller
{
	public function __construct()
    {
            parent::__construct();
            $this->load->model('Rptcollection_model');
	...
	public function index($argCustomerRowId=-1)
	{
		...
		{
			if($this->Util_model->getRight($this->session->userRowId,'Collection Report')==0)
			{
			...
			// $data['customers'] = $this->Rptcollection_model->getCustomerList();
			$data['tmp'] = "tmp";
			$this->load->view('RptCollection_view', $data);
			$this->load->view('includes/footer');
		}
	...
	{
		$data['purchase'] = $this->Rptcollection_model->getPurchase();
		$data['sale'] = $this->Rptcollection_model->getSale();
		$data['quotation'] = $this->Rptcollection_model->getQuotation();
		$data['cashSaleCollection'] = $this->Rptcollection_model->getCashSaleCollection();
		$data['cashSaleExp'] = $this->Rptcollection_model->getCashSaleExp();
	...
	public function printToExcel()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('E5', 'Dr.');
		$objPHPExcel->getActiveSheet()->setCellValue('F5', 'Cr.');

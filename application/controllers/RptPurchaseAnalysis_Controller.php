...
class RptPurchaseAnalysis_Controller extends CI_Controller
{
	public function __construct()
    {
            parent::__construct();
            $this->load->model('Rptpurchaseanalysis_model');
	...
	public function index($argCustomerRowId=-1)
	{
		...
		{
			if($this->Util_model->getRight($this->session->userRowId,'Purchase Analysis')==0)
			{
			...
			$MenuRights['notifications'] = $this->Util_model->getNotifications();
			$this->load->view('includes/menu4admin', $MenuRights);
			$data['itemGroups'] = $this->Rptpurchaseanalysis_model->getItemGroups();
			$this->load->view('RptPurchaseAnalysis_view', $data);
			$this->load->view('includes/footer');
		}
	...
	public function showData()
	{
		...
		$data['records'] = $this->Rptpurchaseanalysis_model->getDataForReport();
 ...
 //        // $myTableRows = strlen($myTableData);
	...
	// 	$data['records'] = $this->Rptpurchaseanalysis_model->getSaleDetail();
	// 	$data['recordsSr'] = $this->Rptpurchaseanalysis_model->getSaleDetailSr();
	// 	echo json_encode($data);
	// }

	// public function getPurchaseDetail()
	// {
	// 	$data['records'] = $this->Rptpurchaseanalysis_model->getPurchaseDetail();
	// 	echo json_encode($data);
	// }
}

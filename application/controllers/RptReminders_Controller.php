...
class RptReminders_Controller extends CI_Controller
{
	public function __construct()
    {
            parent::__construct();
            $this->load->model('Rptreminders_model');
	...
	public function index()
	{
		if ($this->session->isLogin===True && $this->session->session_id != '') /*if logged in*/
		{
			...
			$MenuRights['notifications'] = $this->Util_model->getNotifications();
            $this->load->view('includes/menu4admin', $MenuRights);  
			// $data['customers'] = $this->Rptreminders_model->getCustomerList();
			$this->load->view('RptReminders_view');
			$this->load->view('includes/footer');
		}
	...
	{
		// $data['opBal'] = $this->Rptreminders_model->getOpeningBal();
		$data['records'] = $this->Rptreminders_model->getDataForReport();
		echo json_encode($data);
	}

	public function saveDateNew()
	{
		$this->Rptreminders_model->saveDateNew();

...
class RptSale_Controller extends CI_Controller
{
	public function __construct()
    {
            parent::__construct();
            $this->load->model('Rptsale_model');
	...
	public function index()
	{
		...
		{
			if($this->Util_model->getRight($this->session->userRowId,'Sale Report')==0)
			{
			...
			$MenuRights['notifications'] = $this->Util_model->getNotifications();
			$this->load->view('includes/menu4admin', $MenuRights);
			// $data['parties'] = $this->Rptsale_model->getPartyList();
			$data['tmp'] = "tmp";
			$this->load->view('RptSale_view', $data);
			$this->load->view('includes/footer');
		}
	...
	{
		$data['records'] = $this->Rptsale_model->getDataForReport();
		echo json_encode($data);
	}

	public function getProducts()
	{
		$data['products'] = $this->Rptsale_model->getProducts();
		echo json_encode($data);
	}
	...
	public function printToExcel()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', "Sale Report");
